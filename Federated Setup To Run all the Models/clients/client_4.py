...
images = np.load(os.path.join(CURRENT_DIR, 'client_4_images.npy'))
masks = np.load(os.path.join(CURRENT_DIR, 'client_4_masks.npy'))
...
class CifarClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        checkpoint = ModelCheckpoint("best_model4.h5", monitor="val_loss", save_best_only=True, verbose=1)
        ...
        plt.title(f"Client 4 - Round {self.round}")
        plt.xlabel("Epochs")
        plt.savefig(f"Loss_accu_image_fed_round_client4_{self.round}.png")
        self.round += 1
...
np.save('Y_pred4.npy', Y_pred_bin)
