# ---------------------------------------------
# 📚 IMPORT LIBRARIES (cleaned and structured)
# ---------------------------------------------
import flwr as fl
import numpy as np
import pandas as pd
...
# ---------------------------------------------
# 📁 LOAD DATA FROM CLIENT 2 AND SPLIT
# ---------------------------------------------
images = np.load('client_2_images.npy')   # Input images
masks = np.load('client_2_masks.npy')     # Corresponding masks
...
# Step 2: Temp split into Validation (15%) and Test (15%)
...
# ---------------------------------------------
# 🌐 FEDERATED LEARNING CLIENT (Client 2)
# ---------------------------------------------
class CifarClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # Callbacks for saving and learning rate adjustment
        checkpoint = ModelCheckpoint("best_model2.h5", monitor="val_loss", save_best_only=True, verbose=1)
        ...
        plt.xlabel('Epochs')
        plt.savefig(f"Loss_accu_image_fed_round_client2{self.index1}.png")
        self.index1 += 1
...
# Start the Flower client for Client 2
...
# Compute per-sample evaluation metrics
metrics_list = []
...
# ---------------------------------------------
# 💾 SAVE TEST AND PREDICTION RESULTS
# ---------------------------------------------
np.save('X_test2.npy', X_test)
np.save('Y_test2.npy', y_test)
np.save('Y_pred2.npy', Y_pred_bin)
